// Código separado de brocos2.html
...
function criarJogo(boardId, barraId, bolaId, controles) {
    ...
    let animacao; // vai guardar o ID do requestAnimationFrame
    ...
    function atualizar() {
        ...
        if (blocos.every(b => b === null)) {
            velocidadeX *= 1.1; // opcional: aumenta dificuldade
            velocidadeY *= 1.1;
            criarBlocos(); // cria nova rodada de blocos
        }
        // bola caiu -> jogador perde
        if (bolaY > board.clientHeight - 1) {
            jogoAtivo = false; // para o loop
            cancelAnimationFrame(animacao); // cancela a animação
            document.getElementById("game-over-popup").style.display = "flex";
            document.getElementById("popup-message").textContent = `O ${boardId} perdeu!`;
        }
        ...
        if (jogoAtivo) {
            animacao = requestAnimationFrame(atualizar);
        }
    }
...
criarJogo('Jogador 2', 'barra2', 'bola2', {
    esq: 'ArrowLeft',
    dir: 'ArrowRight'
}); // Player 2

// Popup controls are inline in HTML via onclick attributes (keeps behavior)
